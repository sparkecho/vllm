...
__global__ void __launch_bounds__(1024, VLLM_BLOCKS_PER_SM(1024))
    rms_norm_row_ss_2d(const int num_tokens, const int hidden_size,
  ...
  for (int row_idx = blockIdx.x; row_idx < sf_rows; row_idx += gridDim.x) {
    ...
    float variance = 0.0f;
    for (int col_idx = blockIdx.y * blockDim.x + threadIdx.x;
         col_idx < vecs_per_row_padded;
         col_idx += blockDim.x * gridDim.y) {
      const int elem_idx = col_idx * CVT_FP4_ELTS_PER_THREAD;
      PackedVecT vec{};

      bool valid = (row_idx < num_tokens) && (elem_idx < hidden_size);
    ...
    if (threadIdx.x == 0) {
      atomicAdd(&row_ss[row_idx], variance);
    }
  }
}
...
__global__ void __launch_bounds__(1024, VLLM_BLOCKS_PER_SM(1024))
    rms_norm_cvt_fp16_to_fp4_2d(const int num_tokens, const int hidden_size,
                                scalar_t const* __restrict__ input,
                                scalar_t const* __restrict__ weight,
                                float const* __restrict__ scale,
                                const float epsilon,
                                float const* __restrict__ row_ss,
  ...
  for (int row_idx = blockIdx.x; row_idx < sf_rows; row_idx += gridDim.x) {
    if (threadIdx.x == 0) {
      s_rms_inv = rsqrtf(
          row_ss[row_idx] / static_cast<float>(hidden_size) + epsilon);
    }
    __syncthreads();
    ...
    for (int col_idx = blockIdx.y * blockDim.x + threadIdx.x;
      ...
      const bool valid = (row_idx < num_tokens) && valid_col;
...
template <typename scalar_t>
__global__ void __launch_bounds__(1024, VLLM_BLOCKS_PER_SM(1024))
    fused_add_rms_norm_row_ss_2d(const int num_tokens, const int hidden_size,
                                 scalar_t const* __restrict__ input,
...
void rms_norm_nvfp4_quant_sm1xxa(
  ...
  int grid_y = vllm::div_round_up(vecs_per_row_padded, static_cast<int>(block.x));
  int grid_x = std::min(
      effective_rows,
      std::max(1, (multi_processor_count * num_blocks_per_sm) / grid_y));
  dim3 grid(grid_x, grid_y);

  auto row_ss = torch::zeros({effective_rows},
                             input.options().dtype(at::kFloat));

  VLLM_DISPATCH_HALF_TYPES(
      input.scalar_type(), "rms_norm_nvfp4_quant_kernel", [&] {
        using cuda_type = vllm::CUDATypeConverter<scalar_t>::Type;
        vllm::rms_norm_row_ss_2d<cuda_type><<<grid, block, 0, stream>>>(
            num_tokens, hidden_size,
            reinterpret_cast<cuda_type const*>(input.data_ptr()),
            row_ss.data_ptr<float>());
        vllm::rms_norm_cvt_fp16_to_fp4_2d<cuda_type>
            <<<grid, block, 0, stream>>>(
                num_tokens, hidden_size,
                reinterpret_cast<cuda_type const*>(input.data_ptr()),
...
void fused_add_rms_norm_nvfp4_quant_sm1xxa(
  ...
  VLLM_DISPATCH_HALF_TYPES(
      input.scalar_type(), "fused_add_rms_norm_nvfp4_quant_kernel", [&] {
        using cuda_type = vllm::CUDATypeConverter<scalar_t>::Type;
        vllm::fused_add_rms_norm_row_ss_2d<cuda_type><<<grid, block, 0, stream>>>(
            num_tokens, hidden_size,
            reinterpret_cast<cuda_type const*>(input.data_ptr()),
        ...
        vllm::rms_norm_cvt_fp16_to_fp4_2d<cuda_type>
            <<<grid, block, 0, stream>>>(
                num_tokens, hidden_size,
                reinterpret_cast<cuda_type const*>(residual.data_ptr()),
                reinterpret_cast<cuda_type const*>(weight.data_ptr()),
                scale.data_ptr<float>(), static_cast<float>(epsilon),
                row_ss.data_ptr<float>(),
                reinterpret_cast<uint32_t*>(output.data_ptr()),
